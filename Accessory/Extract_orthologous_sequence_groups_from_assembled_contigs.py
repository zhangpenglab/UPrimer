...
One is the reference nucleotide sequence file and it needs to be named 'Reference_nucleotide_sequences_for_target_regions.fasta'.
Another is the reference peptide sequence file. It needs to be named 'Reference_peptide_sequences_for_target_regions.fasta'.
The format of reference sequence id is '>Lep1'.

3) A folder named 'pep_for_exonerate' that contains the file 'Reference_peptide_sequences_for_target_regions.fasta'
...
def tblastn(contigDir):
    ...
    for i in range(0,len(list_contig)):
        ...
        if os.path.isfile(contig_file) and contig_file.split('.')[-1]=='fasta':
            sample=os.path.basename(contig_file).split('.')[0]
            reference=os.path.join(contigDir,os.path.pardir,'reference','Reference_peptide_sequences_for_target_regions.fasta')
...
def get_best_Hit_info(contigDir):
    ...
    for filename in filenames:
        ...
        refername=os.path.join(contigDir,os.path.pardir,'reference','Reference_peptide_sequences_for_target_regions.fasta')
        referdic={}
        genedic={}   
...
def separate_pep_to_single_fasta(contigDir):
    ...
    files=glob.glob(os.path.join(pathout,'Reference_peptide_sequences_for_target_regions.fasta'))
    for fi in files:
...
def add_reference(contigDir):
    ...
    if not os.path.exists(pathout):
        os.mkdir(pathout)
    refSeq=SeqIO.parse(os.path.join(contigDir,os.path.pardir,'reference','Reference_nucleotide_sequences_for_target_regions.fasta'),'fasta')
    ref_dic={}
    for ref in refSeq:
...
def filt_gene_length_and_translate(contigDir):########modified by ljx
    cutoff=0.4#####
    ...
    for filename in filenames:
        ...
        if taxa>1:
            ...
            for record in SeqIO.parse(filename,'fasta'):
                if record.id.split('_')[0]!='reference':######new add
