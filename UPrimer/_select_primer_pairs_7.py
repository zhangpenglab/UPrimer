...
def Extract_reference_sequences(file_in, fasta_out, dir, num, taxa_group):
    ...
    for record in SeqIO.parse(ref_cds_file[0],'fasta'):
        if '<unknown id> ' in record.description and '_unknown' not in record.description:
            pre_msa_name = str(record.description).replace('<unknown id> ', '')
        ...
        elif '<unknown id> ' in record.description and '_unknown' in record.description:
            pre_msa_name = str(record.description).replace('<unknown id> ', '').replace('_unknown','')
