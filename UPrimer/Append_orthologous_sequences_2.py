...
def Create_sequence_dictionary(species_list,cut_length):
	...
	## <unknown id> XP_038957404.1_GeneID120100082_LOC120100082_NC_051336.1_1335317_1335628_-1_retinoic acid early-inducible protein 1-gamma-like	
...
def Extract_orthologous_sequence_groups(line, dic_list, title, min_number):
	...
	if number >= min_number:
		genes = line.strip().split('\t')
                ...
                #Format: ><unknown id> XP_047753575.1_GeneID71966332_GeneID71966332_NC_062999.1_100866_101500_1_ISWI chromatin-remodeling complex ATPase ISW2		
		if '<unknown id> ' in genes[0] and '_unknown' not in genes[0]:
                        pre_msa_name = genes[0].replace('<unknown id> ', '')
                ...
                #Format: ><unknown id> EUB53855.1_unknown_unknown_APAU02000760.1_556_895_-1_hypothetical protein
                elif '<unknown id> ' in genes[0] and '_unknown' in genes[0]:
                        pre_msa_name = genes[0].replace('<unknown id> ', '').replace('_unknown','') #>EUB53855.1_APAU02000760.1_556_895_-1_hypothetical protein
						...
						if i == 0:
                                                        if '<unknown id> ' in genes[0]:
                                                                ...
                                                                if '*' in str(all_length_seq.translate()):
                                                                        ...
                                                                        record_nucl = '>' + title[i].strip() + '_' + str(genes[0]).split('<unknown id> ')[1].split('_')[0] + '_' +\
                                                                                                  str(genes[0]).split('<unknown id> ')[1].split('_')[1] + '_' +\
                                                                                                  str(genes[0]).split('<unknown id> ')[1].split('_')[6].replace('<','').replace('>','').replace('/','_').replace('(','').replace(')','') + '_' +\
                                                                                                  str(genes[0]).split('<unknown id> ')[1].split('_')[7].replace('<','').replace('>','').replace('/','_').replace('(','').replace(')','') + linesep + str(all_length_seq)
                                                                        fh_out_nucl.write(record_nucl + linesep)
                                                                        record_pep = '>' + title[i].strip() + '_' + str(genes[0]).split('<unknown id> ')[1].split('_')[0] + '_' +\
                                                                                                  str(genes[0]).split('<unknown id> ')[1].split('_')[1] + '_' +\
                                                                                                  str(genes[0]).split('<unknown id> ')[1].split('_')[6].replace('<','').replace('>','').replace('/','_').replace('(','').replace(')','') + '_' +\
                                                                                                  str(genes[0]).split('<unknown id> ')[1].split('_')[7].replace('<','').replace('>','').replace('/','_').replace('(','').replace(')','') +  linesep + str(all_length_seq.translate())
